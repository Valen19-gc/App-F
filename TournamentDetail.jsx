...
export default function TournamentDetail() {
  ...
  async function fetchTournament() {
    ...
    setTournament(data)
    // Set active round to first incomplete
    const firstIncomplete = data.rounds?.findIndex(r =>
      r.matches.some(m => !m.result)
    )
    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.roundsPanel}>
          ...
          {round && (
            <div className={styles.roundDetail}>
              <div className={styles.restBadge}>
                😴 Descansa: <strong>{round.resting?.name}</strong>
              </div>
              ...
              {foosMatch && (
                <MatchCard
                  ...
                  icon="⚽"
                  label="FUTBOLÍN"
                  onSave={(w, s1, s2) => saveResult(foosMatch.id, w, s1, s2)}
                />
              )}
              ...
              {billMatch && (
                <MatchCard
                  ...
                  icon="🎱"
                  label="BILLAR"
                  onSave={(w, s1, s2) => saveResult(billMatch.id, w, s1, s2)}
                />
              )}
            </div>
          )}
        ...
        <div className={styles.leaderboardPanel}>
          <div className={styles.panelTitle}>CLASIFICACIÓN</div>
      ...
      {!hasResult && (
        <div className={styles.resultForm}>
          ...
          <div className={styles.winBtns}>
            <button className={styles.winBtn} data-team="1" onClick={() => onSave(1, s1, s2)}>
              Gana {team1.map(p => p.name).join('+')}
            </button>
            <button className={styles.winBtn} data-team="2" onClick={() => onSave(2, s1, s2)}>
